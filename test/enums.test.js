...
describe("enum", () => {
  describe("core", () => {
    ...
    test("the values cannot be changed after creation of enum", () => {
      let weekDays = enums()(wd);
      expect(weekDays.monday).toBe(0);
      let error;
      try {
        weekDays.monday = "not zero";
      } catch (err) {
        error = err;
      }
      expect(weekDays.monday).toBe(0);
      expect(error.message).toEqual(
        "Cannot assign to read only property 'monday' of object '#<Enum>'"
      );
    });

    test("the values cannot be added after creation of enum", () => {
      let weekDays = enums()(wd);
      let error;
      try {
        weekDays.newDay = "new weekday";
      } catch (err) {
        error = err;
      }
      expect(error.message).toEqual(
        "Cannot add property newDay, object is not extensible"
      );
    });
  });
